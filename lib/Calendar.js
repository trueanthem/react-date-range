...
function getMonthRange(dayMoment) {
  var startOfMonth = dayMoment.clone().startOf('month');
  var endOfMonth = dayMoment.clone().endOf('month');
  return {
...
function isStartOfWeekMonth(dayMoment) {
  var _getMonthRange = getMonthRange(dayMoment);

  var startOfMonth = _getMonthRange.startOfMonth;

  return dayMoment.day() === 0 || isSameDate(dayMoment, startOfMonth);
}

function isEndOfWeekMonth(dayMoment) {
  var _getMonthRange2 = getMonthRange(dayMoment);

  var endOfMonth = _getMonthRange2.endOfMonth;
...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    value: function componentWillReceiveProps(newProps) {
      ...
      var range = newProps.range;
      var offset = newProps.offset;
      ...
      var newDate = (0, _utilsParseInputJs2['default'])(newProps.date, format);
      var oldDate = (0, _utilsParseInputJs2['default'])(this.props.date, format);

      if (startDate && endDate && startDate.isSame(endDate)) {
        return;
      }

      if (newProps.date && this.props.date && newDate.isSame(oldDate)) {
        return;
      }

      this.setState({
        date: newDate,
        shownDate: (range && endDate || newDate).clone().add(offset, 'days')
      });
    }
  }, {
    key: 'getShownDate',
    value: function getShownDate() {
      ...
      var shownDate = link ? link.clone().add(offset, 'days') : this.state.shownDate;
    ...
    key: 'renderMonthButton',
    value: function renderMonthButton(onlyClasses, styles, defaultButton, customButton) {
      var btn = styles[customButton];
      if (btn) {
        return btn;
      }
      return _react2['default'].createElement('i', { style: onlyClasses ? undefined : _extends({}, styles['MonthArrow'], styles[defaultButton]) });
    }
  }, {
    key: 'renderMonthAndYear',
    value: function renderMonthAndYear(classes) {
      var shownDate = this.getShownDate();
      var month = shownDate.format("MMM.");
    ...
    value: function renderDays(classes) {
      ...
      return days.map(function (data, index) {
        ...
        var isInRangeFirstOfRow = isStartOfWeekMonth(dayMoment) && (isInRange || isEndEdge) || isStartEdge;
        var isInRangeLastOfRow = isEndOfWeekMonth(dayMoment) && (isInRange || isStartEdge) || isEndEdge;
        var isVisible = dayMoment.month() === monthNumber;
